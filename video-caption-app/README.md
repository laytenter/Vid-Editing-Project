...
- Generate `.srt` and `.vtt` captions locally with the bundled Whisper runtime
...
- `bin/` - local runtime binaries (FFmpeg, ffprobe, and Whisper runtime files)
- `scripts/` - helper scripts such as static file copying
- `.runtime-staging/` - local staging area used when auditing or updating runtime binaries
...
This repository currently includes Windows runtime binaries in `bin/`.
...
The bundled FFmpeg/ffprobe binaries have been updated to a stable Gyan FFmpeg essentials build. Previous FFmpeg binaries may be kept locally as `.previous.exe` backups during development.
