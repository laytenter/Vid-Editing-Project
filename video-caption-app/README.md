...
**Status:** Early development / experimental. The app currently runs with `npm start` and is not packaged yet.
...
This repository keeps the app source and smaller runtime files in Git, but does not commit oversized FFmpeg/ffprobe executables because GitHub rejects blobs over 100 MB. Keep those files locally in `bin/`.
...
`scripts/setup-windows.ps1` creates the local runtime folders and downloads the Whisper model. It does not download FFmpeg; use a Windows FFmpeg build such as Gyan FFmpeg essentials and place `ffmpeg.exe` and `ffprobe.exe` in `bin/`.
...
- Currently Windows-focused because bundled FFmpeg and Whisper binaries are Windows builds.
